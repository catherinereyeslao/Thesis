...
public class ChapterEight extends ChapterCore{
	...
	private Texture introBg, paghahabiBg, pagpintaBg, panitikanBg, sayawBg,
			...
			pagsayawBalloon1, pagsayawBalloon2, pagsayawBalloon3, boxTexture, gameBg, nextTexture,
            habi1Texture, habi2Texture, painter1Texture, painter2Texture;
    private TextureAtlas dancerAtlas, readerAtlas;
    private Animation dancerAnimation, readerAnimation;
    private Sprite box, box2, box3, box4, next, character, dancer, reader;
    private boolean boxTouched, box2Touched, box3Touched, box4Touched, renderedDancer = false;
	...
	public void setUp(int screenW, int screenH){
        ...
        character.setSize((character.getWidth() * getButtonScale() * 1.75f), (character.getHeight() * getButtonScale()) * 1.75f);

        dancerAtlas = new TextureAtlas("chapters/chapter8/characters/dancer.atlas");
        dancerAnimation = new Animation(0.5f, dancerAtlas.getRegions());
        dancer = new Sprite(dancerAnimation.getKeyFrames()[0]);
        dancer.setSize((dancer.getWidth() * getButtonScale()) * 1.8f, (dancer.getHeight() * getButtonScale()) * 1.8f);

        readerAtlas = new TextureAtlas("chapters/chapter8/characters/reader.atlas");
        readerAnimation = new Animation(0.5f, readerAtlas.getRegions());
        reader = new Sprite(readerAnimation.getKeyFrames()[0]);
        reader.setSize((reader.getWidth() * getButtonScale()) * 1.3f, (reader.getHeight() * getButtonScale()) * 1.3f);

        boxTexture = new Texture("box.png");

        box = new Sprite(boxTexture);
        box.setSize((box.getWidth() * getButtonScale()) / 1.12f, (box.getWidth() * getButtonScale()) / 1.8f);
        ...
        box2.setSize((box2.getWidth() * getButtonScale()) / 1.292f, (box2.getWidth() * getButtonScale()) / 1.48f);
        ...
        box3.setSize((box3.getWidth() * getButtonScale()) / 1.12f, (box3.getWidth() * getButtonScale()) / 1.418f);
        ...
        box4.setSize((box4.getWidth() * getButtonScale()) / 1.32f, (box4.getWidth() * getButtonScale()) / 1.31f);
    ...
    private void assetManager() {
        ...
        case 15:
            ...
            break;
        case 16:
            if (boxTouched) correctAnswers++;
            if (box2Touched) correctAnswers++;
            if (box3Touched) correctAnswers++;
            if (box4Touched) correctAnswers++;
    ...
    public int touchDown(float x, float y) {
        if (chapterSection == lastChapterSection)
            displayLastSectionButtons(8, 2, x, y);
